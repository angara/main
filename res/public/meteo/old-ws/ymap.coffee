#
#   angara.ws:  Yandex.Map
#   client side
#

d02 = (d) -> if d < 10 then "0"+d else ""+d

ddmmyy_hhmm = (t) ->
    return "??.??.?? ??:??" if not t
    ""+d02(t.getDate())+"."+d02(t.getMonth()+1)+"."+d02(t.getYear()-100)+
    " "+d02(t.getHours())+":"+d02(t.getMinutes())

MON_RU = ["Янв","Фев","Мар","Апр","Май","Июн","Июл","Авг","Сен","Окт","Ноя","Дек"]

mmmdd_hhmm = (t) ->
    return "???.?? ??:??" if not t
    ""+MON_RU[t.getMonth()]+"."+d02(t.getDate())+" "+d02(t.getHours())+":"+d02(t.getMinutes())
#-

t_deg = (t) ->
    t = Math.round(t)
    return (if t > 0 then "+"+t else ""+t)+"&deg;"
#-

st_hint = (st) ->
    return "<b style='font-size: 12px;'>"+st.title+"</b>"+
        "<br/>"+
        "<span style='color:#77b; margin: auto 4px;'>"+
            mmmdd_hhmm(new Date(st.last.ts))+"</span>"
#-

st_balloon = (st) ->
    return "<div>"+(st.addr || st.title || "")+"</div>"+
        "<div style='padding: auto 8px; text-align: center;'>"+
        "<span style='font-size: 15px;'><b>"+t_deg(st.last.t)+"</b>C</span>"+
        "<span style='color:#77b; margin-left: 8px; font-size: 12px;'>"+
            mmmdd_hhmm(new Date(st.last.ts))+"</span>"+
        "</div>"
#-

add_marker = (st) ->
    window.ymap.geoObjects.add( new ymaps.Placemark( st.ll,
        {
            id: st.id
            type: 'meteo_marker'
            iconContent: t_deg(st.last.t)
            hintContent: st_hint(st)
            balloonContent: st_balloon(st)
            hideIcon: false
        },
        {
            preset: 'twirl#lightblueStretchyIcon'
        }
    ))
#-

update_st = (data) ->
    ymap = window.ymap
    return unless ymap? and data

    dm = {}
    for d in data
        dm[d._id] = d
    #

    ymap.geoObjects.each (gobj) ->
        return if gobj.properties.get("type") isnt 'meteo_marker'
        st_id = gobj.properties.get 'id'
        d = dm[st_id]
        if not d
            ymap.geoObjects.remove gobj
            return
        gobj.properties.set 'iconContent', t_deg(d.last.t)
        gobj.properties.set 'hintContent', st_hint(d)
        gobj.properties.set 'balloonContent', st_balloon(d)
        d['updated'] = true
    #

    add_marker(d) for d in data when not d.updated
    return
#-


$ ->
    ymaps_api = "http://api-maps.yandex.ru/2.0-stable/?lang=ru_RU"+
                    "&load=package.standard&coordorder=longlat"

    METEO = window.meteo

    $.getScript ymaps_api, () ->
        ymaps.ready () ->
            ymap = new ymaps.Map(METEO.map_div_id, {
                center: [104.276084, 52.270944]
                zoom: 10
                type: 'yandex#hybrid'
            })
            window.ymap = ymap

            # init other stuff

            ymap.controls.add 'zoomControl' # , {right : '35px'}
            ymap.controls.add( new ymaps.control.TypeSelector() )
            ymap.controls.add 'mapTools'
            ymap.controls.add 'scaleLine'

            METEO.update_st = update_st
            update_st(METEO.last_st_data) if METEO.last_st_data
        #-
    #-
#

#.
